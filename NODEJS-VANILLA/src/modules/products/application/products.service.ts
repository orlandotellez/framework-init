...
function mapProductToResponse(product: RichProductEntity): IProductResponse {
  return {
    ...
    created_at: product.created_at instanceof Date ? product.created_at.toISOString() : product.created_at as string,
    updated_at: product.updated_at instanceof Date ? product.updated_at.toISOString() : product.updated_at as string,
  };
}
...
export const createProductService = (repository: IProductRepository) => ({
  ...
  getById: async (id: string, storeId?: string): Promise<IProductResponse> => {
    ...
    return mapProductToResponse(product as RichProductEntity);
  },
