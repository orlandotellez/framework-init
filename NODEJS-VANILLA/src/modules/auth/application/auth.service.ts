...
export const createAuthService = (repository: IAuthRepository) => ({
  register: async (data: IRegisterPayload, storeId: string): Promise<IAuthResponse> => {
    ...
    if (existingUser) throw new ConflictError("Email already registered in this store");
  ...
  registerStore: async (data: IRegisterStorePayload): Promise<IRegisterStoreResponse> => {
    ...
    if (existingStore) throw new ConflictError("A store with this name already exists");
    ...
    const result = await prisma.$transaction(async (tx) => {
      const store = await tx.store.create({
        data: { name: storeName, address: storeAddress || "", phone: storePhone || "" },
      });

      const user = await tx.user.create({
        data: { name: adminName, email: adminEmail, role: "admin", email_verified: true, store_id: store.id },
      });

      await tx.account.create({
        data: { account_id: user.id, provider_id: "credentials", user_id: user.id, password: hashedPassword },
      });

      await tx.settings.create({
        data: { store_id: store.id, name: storeName, address: storeAddress || "", phone: storePhone || "", tax_rate: 16, low_stock_threshold: 5 },
      });
  ...
  resendVerification: async (email: string): Promise<IVerificationResponse> => {
    const user = await repository.user.findByEmail(email);
    if (!user) return { message: "If the email exists, a new verification code has been sent", expiresAt: new Date(Date.now() + VERIFICATION_CODE_EXPIRY) };
